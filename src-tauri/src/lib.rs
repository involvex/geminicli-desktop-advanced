...
#[tauri::command]
async fn start_session(_session_id: String) -> Result<(), String> {
...
async fn initialize_session(
    ...
    // Store the session
    {
        let mut processes = state
            .processes
            .lock()
            .map_err(|_err| "Failed to lock processes")?;
        processes.insert(
            session_id.clone(),
            PersistentSession {
        ...
        "pushToolCall" => {
            if let Ok(params) = serde_json::from_value::<PushToolCallParams>(request.params) {
                ...
                *tool_call_id += 1;
                ...
                let _ =
                    app_handle.emit(&format!("gemini-tool-call-{session_id}"), &tool_call_data);
            }
        }
        "updateToolCall" => {
...
async fn send_message(
    ...
    message_sender
        .send(request_json)
        .map_err(|_err| "Failed to send message to session")?;
...
async fn generate_conversation_title(
    ...
    if let Some(stdin) = child.stdin.take() {
        ...
        stdin
            ...
            .await
            .map_err(|e| format!("Failed to write prompt to gemini stdin: {e}"))?;
        stdin
            .shutdown()
            .await
            .map_err(|e| format!("Failed to close gemini stdin: {e}"))?;
    }
...
async fn debug_environment() -> Result<String, String> {
    ...
    let home = std::env::var("HOME").unwrap_or_else(|_err| {
        std::env::var("USERPROFILE").unwrap_or_else(|_err| "HOME not found".to_string())
    });
