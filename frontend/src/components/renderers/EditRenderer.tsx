...
import { Edit3, Check, X, ChevronRight, ChevronDown } from "lucide-react";
...
export function EditRenderer({ toolCall, onConfirm, hasConfirmationRequest }: EditRendererProps) {
  ...
  const getEditInfo = () => {
    ...
    if (toolCall.parameters?.edits && Array.isArray(toolCall.parameters.edits)) {
      const edits = toolCall.parameters.edits as EditResult['edits'];
      return {
        type: 'multi' as const,
        edits: edits || [],
        totalAdditions: edits?.reduce((sum, edit) => sum + (result.additions || 0), 0) || 0,
        totalDeletions: edits?.reduce((sum, edit) => sum + (result.deletions || 0), 0) || 0
      };
    }
  ...
  const getTotalChanges = () => {
    if (editInfo.type === 'single') {
      ...
      if (editInfo.additions === 0 && editInfo.deletions === 0 && editInfo.oldText && editInfo.newText) {
      ...
      return { additions: editInfo.totalAdditions, deletions: editInfo.totalDeletions };
    }
  };
      ...
      <Card className={`${
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <CardTitle className="text-sm font-mono">
                {editInfo.type === 'single' ? editInfo.filePath : '(multiple files)'}
                ...
                <>
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end" className="w-48">
                      <DropdownMenuItem onClick={() => onConfirm?.(toolCall.id, "alwaysAllow")}>
                        Always allow edits from this agent
                      </DropdownMenuItem>
                      <DropdownMenuItem onClick={() => onConfirm?.(toolCall.id, "alwaysAllowTool")}>
                        Always allow this tool
                      </DropdownMenuItem>
                      <DropdownMenuItem onClick={() => onConfirm?.(toolCall.id, "alwaysAllowMcpServer")}>
        ...
        <CardContent>
          <DiffViewer
            ...
            fileName={editInfo.type === 'single' ? editInfo.filePath : '(multiple files)'}
