...
export function ReadFileRenderer({ toolCall }: ReadFileRendererProps) {
  ...
  const getFileInfo = (): { isMultiple: boolean; files: string[]; displayPath: string; isPatternBased: boolean } => {
    let files: string[] = [];
    ...
    if (result?.markdown && typeof result.markdown === 'string') {
      ...
      if (markdown.includes('ReadManyFiles Result') || markdown.includes('Processed Files:')) {
        isPatternBased = false; // It's resolved files now, not patterns
        
        // Extract processed files from markdown
        const processedMatch = markdown.match(/\*\*Processed Files:\*\*\n((?:- `.+`\n?)*)/);
      ...
      if (toolCall.inputJsonRpc) {
        ...
        if (params.locations && Array.isArray(params.locations) && params.locations.length > 0) {
          files = params.locations.map((loc: unknown) => 
            (typeof loc === 'object' && loc !== null && 'path' in loc) ? (loc as { path: string }).path : String(loc)
          );
        }
        // If locations is empty but we have a label with patterns, parse the label
        else if (params.locations && Array.isArray(params.locations) && params.locations.length === 0 && params.label) {
          isPatternBased = true;
        ...
        else if (params.file || params.path || params.filename || params.filePath) {
          const singleFile = params.file || params.path || params.filename || params.filePath;
          files = [singleFile];
        }
      }
    } catch {
  ...
  // Get status message
  const getStatusMessage = (fileInfo: { isMultiple: boolean; files: string[] }): string => {
    if (result.error) {
